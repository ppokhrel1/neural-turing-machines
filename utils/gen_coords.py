...
chart = {'ALA' : 1, 'GLN':2, "LEU":3, "SER":4, "ARG":5,"GLU":6, "LYS":7, "THR":8,'ASN':9,"GLY":10, 'MET':11, "TRP":12, "ASP":13, "HIS":14, "PHE":15, "TYR":16, "CYS":17, "ILE":18, "PRO":19, "VAL":20, "UHK":21}
    ...
    with open(fileName, "r") as f:
        for line in f:
            ...
            if myline[0] == 'ATOM' and len(myline[3]) > 2: #check only for amino acids, not the RNA molecules
                #print myline[3]
                residues.append(chart[(myline[3])])
                #print myline[5],
                #print myline[6],
                #print myline[7]
                if (cmp(prevcoords,[]) == 0):
                ...
                #for x in range(scale_factor):
                    ...
                    #coords.pop()
                    #print len(coords)
    myres = residues[: len(residues)-scale_factor]
    mycoords = coords[: len(residues) - scale_factor]

    # we will turn the amino acids to h-p
    myres = array(myres, dtype='float32')
    mycoords = array(mycoords, dtype='float32')
    print len(myres) / 9
    print len(myres) % 9
    myres = myres.reshape(len(myres)/9, 9)
    mycoords = mycoords.reshape(len(mycoords)/9, 9)
